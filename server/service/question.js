...
exports.update = function (req, res) {
    ...
    //on question update, updates the tags
    //Step 1: Get all tag connections
    //Step 2: Cross check the tag ids
    //Step 3: Add/delete the connections
    var newTags = [];
    var createTagConnection = function (questionId, tagId) {
        ...
        connection.save(function () { }, function () { });
    };
    var deleteTagConnection = function (connId) {
        var connection = new Appacitive.Connection({ relation: 'question_tag', __id: connId });
        connection.del(function () { }, function () { });
    };
    var updateTags = function (callback) {
        ...
        aQuestion.fetchConnectedArticles({
            ...
            var tags = aQuestion.children['question_tag'];
            tags.forEach(function (tag) {
                var tagIndex = question.tags.indexOf(tag.id());
                //don't do anything
                if (tagIndex >= 0) {
                ...
                deleteTagConnection(tag.connection.id());
                aQuestion.removeTag(tag.get('name'));
            });

            //Create new connections
            for (var count = 0; count < question.tags.length; count++) {
                createTagConnection(aQuestion.id(), question.tags[count]);
            }
            for (var count = 0; count < newTags.length; count++) {
                aQuestion.addTag(newTags[count]);
            }
    ...
    switch (question.action) {
        ...
        case 'do:save':
            //get the question
            //check the ownership
            aQuestion.fetch(function () {
                if (aQuestion.get('__createdby') != state.userid) return res.status(401).json(transformer.toError('access_denied'));
                //update title
